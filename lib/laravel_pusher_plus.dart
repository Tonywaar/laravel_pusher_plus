...
part 'laravel_pusher_plus.g.dart';
...
class Channel {
  final String name;
  final LaravelPusherPlus pusher;
  ...
  void _subscribe() async {
    var args = jsonEncode(BindArgs(instanceId: pusher._instanceId, channelName: name).toJson());
...
class LaravelPusherPlus {
  static const MethodChannel _channel = MethodChannel('dev.tonywaar/pusher');
  final EventChannel _eventChannel = const EventChannel('dev.tonywaar/pusherStream');
  ...
  Future connect({
    ...
    await _channel.invokeMethod('connect', jsonEncode({'instanceId': _instanceId}));
  }

  /// Disconnect the client from pusher
  Future disconnect() async {
    await _channel.invokeMethod('disconnect', jsonEncode({'instanceId': _instanceId}));
  }
  ...
  Future unsubscribe(String channelName) async {
    await _channel.invokeMethod('unsubscribe', jsonEncode({'channelName': channelName, 'instanceId': _instanceId}));
  }
  ...
  void _handleEvent([dynamic arguments]) async {
    ...
    if (message.isEvent) {
      var callback = _eventCallbacks[message.event!.channel + message.event!.event];
      if (callback != null) {
...
class PusherClient extends LaravelPusherPlus {
  PusherClient(
    String appKey,
    PusherOptions options, {
    bool lazyConnect = false,
...
class InitArgs {
  ...
  InitArgs(this.instanceId, this.appKey, this.options, {this.isLoggingEnabled = false});

  factory InitArgs.fromJson(Map<String, dynamic> json) => _$InitArgsFromJson(json);
...
class PusherEventStreamMessage {
  ...
  PusherEventStreamMessage({this.event, required this.instanceId, this.connectionStateChange, this.connectionError});

  factory PusherEventStreamMessage.fromJson(Map<String, dynamic> json) => _$PusherEventStreamMessageFromJson(json);
...
class ConnectionStateChange {
  ...
  factory ConnectionStateChange.fromJson(Map<String, dynamic> json) => _$ConnectionStateChangeFromJson(json);
...
class ConnectionError {
  ...
  factory ConnectionError.fromJson(Map<String, dynamic> json) => _$ConnectionErrorFromJson(json);
