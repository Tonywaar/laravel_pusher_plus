...
public class PusherInstance: PusherDelegate{
    ...
    // Define method call to handle methods called from plugin
    ...
    // Get socket id using [FlutterMethodCall] & [FlutterResult]
                ...
                bindedEvents[bindArgs.channelName + bindArgs.eventName] = channelObj.bind(eventName: bindArgs.eventName, callback: { data in
                    do {
                        if let dataObj = data as? [String : AnyObject] {
                            ...
                            if let eventSinkObj = SwiftLaravelPusherPlusPlugin.eventSink {
...
class StreamHandler: NSObject, FlutterStreamHandler {
    public func onListen(withArguments arguments: Any?, eventSink events: @escaping FlutterEventSink) -> FlutterError? {
        SwiftLaravelPusherPlusPlugin.eventSink = events
        return nil;
    }
